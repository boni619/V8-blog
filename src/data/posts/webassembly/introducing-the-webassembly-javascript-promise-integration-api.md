...
Published 01 July 2024 · Tagged with [WebAssembly](/blog/tags/webassembly)
...
What is ‘JSPI’ for? [#](#what-is-%E2%80%98jspi%E2%80%99-for%3F)
---------------------------------------------------------------
...
On the other hand, many cases C/C++ (and many other languages) applications are originally written against a _synchronous_ API. For example, the Posix `read` function does not complete until the I/O operation is complete: the `read` function _blocks_ until the read is complete.
...
In addition, the WebAssembly export is refactored to return a `Promise` object — instead of the original returned value from the export. This `Promise` object becomes the value returned by the WebAssembly application: when the WebAssembly code is suspended,[\[1\]](#fn1) the exported `Promise` object is returned as the value of the call into WebAssembly.
...
#### Wrapping imports and exports [#](#wrapping-imports-and-exports)
...
Of course, there is a significant amount of internal mechanisms that allow this to happen;[\[2\]](#fn2) but neither the JavaScript language nor WebAssembly itself are changed by the JSPI. Its operations are confined to the boundary between JavaScript and WebAssembly.
...
### How can I use JSPI today? [#](#how-can-i-use-jspi-today%3F)
...
To see all this working, let’s try a simple example. This C program computes Fibonacci in a spectacularly bad way: by asking JavaScript to do the addition, even worse by using JavaScript `Promise` objects to do it:[\[3\]](#fn3)
...
To compile our small demo, we use Emscripten’s `emcc` compiler:[\[4\]](#fn4)
...
1.  If a WebAssembly application is suspended more than once, subsequent suspensions will return to the browser's event loop and will not be directly visible to the web application. [↩︎](#fnref1)
    
2.  For the technically curious, see [the WebAssembly proposal for JSPI](https://github.com/WebAssembly/js-promise-integration/blob/main/proposals/js-promise-integration/Overview.md). [↩︎](#fnref2)
    
3.  Note: we include the complete program below, in Appendix A. [↩︎](#fnref3)
    
4.  Note: you need a version of Emscripten that is ≥ 3.1.61. [↩︎](#fnref4)
