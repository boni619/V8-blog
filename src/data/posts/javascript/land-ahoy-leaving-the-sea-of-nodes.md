...
Published 25 March 2025 · Tagged with [JavaScript](/blog/tags/javascript) [internals](/blog/tags/internals)
...
The birth of Turbofan and Sea of Nodes [#](#the-birth-of-turbofan-and-sea-of-nodes)
===================================================================================
...
But what is Sea of Nodes, really? [#](#but-what-is-sea-of-nodes%2C-really%3F)
=============================================================================
...
![](/_img/leaving-the-sea-of-nodes/CFG-example-1.svg)
...
![](/_img/leaving-the-sea-of-nodes/CFG-example-2.svg)

CFG graph with arithmetic operations that could be reordered
...
![](/_img/leaving-the-sea-of-nodes/Sea-of-Nodes-arith.svg)

Simple Sea of Nodes graph with arithmetic operations
...
So, to enjoy more freedom and actually benefit from Sea of Nodes, Turbofan has another kind of edge, _effect edges_, which impose some ordering on nodes that have side effects. Let’s ignore control flow for now and look at a small example:

![](/_img/leaving-the-sea-of-nodes/Sea-of-Nodes-effects.svg)

Sea of Nodes graph with effectful operations
...
This example is a good showcase of the power of Sea of Nodes compared to CFG: `y = x * c` is only used in the `else` branch thus will freely float to after the `branch` rather than being computed before as was written in the original JavaScript code. This is similar for `arr[0]`, which is only used in the `else` branch, and _could_ thus float after the `branch` (although, in practice, Turbofan will not move down `arr[0]`, for reasons that I’ll explain later).  
For comparison, here is what the corresponding CFG would look like:

![](/_img/leaving-the-sea-of-nodes/CFG-control-and-effects.svg)

CFG graph with control flow and effectful operations
...
And the troubles begin… [#](#and-the-troubles-begin%E2%80%A6)
=============================================================
...
Manually/visually inspecting and understanding a Sea of Nodes graph is hard [#](#manually%2Fvisually-inspecting-and-understanding-a-sea-of-nodes-graph-is-hard)
---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
![](/_img/leaving-the-sea-of-nodes/Sea-of-Nodes-array-concat.png)

Sea of Nodes graph for a simple array concatenation function
...
![](/_img/leaving-the-sea-of-nodes/CFG-array-concat.png)

CFG graph for the same simple array concatenation function
...
Too many nodes are on the effect chain and/or have a control input [#](#too-many-nodes-are-on-the-effect-chain-and%2For-have-a-control-input)
---------------------------------------------------------------------------------------------------------------------------------------------
...
Let’s have a look at a concrete example, and start from the following JavaScript function:
    ...
    function foo(a, b) {  // assuming that `a.str` and `b.str` are strings  return a.str + b.str;}
...
![](/_img/leaving-the-sea-of-nodes/Sea-of-Nodes-string-add.png)

Sea of Nodes graph for a simple string concatenation function
...
*   `1` (`CheckedTaggedToTaggedPointer`): this checks that the 1st input of the function is a pointer and not a “small integer” (see [Pointer Compression in V8](https://v8.dev/blog/pointer-compression)). On its own, it wouldn’t really _need_ an effect input, but in practice, it still needs to be on the effect chain, because it guards the following nodes.
...
Memory operations do not float easily [#](#memory-operations-do-not-float-easily)
---------------------------------------------------------------------------------
...
![](/_img/leaving-the-sea-of-nodes/Sea-of-Nodes-mirror-control-effect.svg)

Sea of Nodes graph where the effect chain mirrors the control chain, leading to effectful operations not floating as freely as one may hope
...
Managing the effect and control chains manually is hard [#](#managing-the-effect-and-control-chains-manually-is-hard)
---------------------------------------------------------------------------------------------------------------------
...
The scheduler is too complex [#](#the-scheduler-is-too-complex)
---------------------------------------------------------------
...
![](/_img/leaving-the-sea-of-nodes/Sea-of-Nodes-switch-case.svg)

Sea of Nodes graph for a simple switch-case
...
Finding a good order to visit the graph is difficult [#](#finding-a-good-order-to-visit-the-graph-is-difficult)
---------------------------------------------------------------------------------------------------------------
...
Cache unfriendliness [#](#cache-unfriendliness)
-----------------------------------------------
...
Control-flow dependent typing is limited [#](#control-flow-dependent-typing-is-limited)
---------------------------------------------------------------------------------------
...
And many other issues… [#](#and-many-other-issues%E2%80%A6)
-----------------------------------------------------------
...
Sea of Nodes: elegant but impractical for JavaScript [#](#sea-of-nodes%3A-elegant-but-impractical-for-javascript)
=================================================================================================================
