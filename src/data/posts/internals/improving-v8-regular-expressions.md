...
Published 04 October 2019 · Tagged with [internals](/blog/tags/internals) [RegExp](/blog/tags/regexp)
...
Tier-up strategy for RegExp [#](#tier-up-strategy-for-regexp)
-------------------------------------------------------------
...
Speeding up the RegExp interpreter [#](#speeding-up-the-regexp-interpreter)
---------------------------------------------------------------------------

### Remove runtime overhead [#](#remove-runtime-overhead)
...
### New dispatch method [#](#new-dispatch-method)
...
### Bytecode peephole optimization [#](#bytecode-peephole-optimization)

Before we talk about bytecode peephole optimization, let’s look at a motivating example.

    const re = /[^_]*/;const str = 'a0b*c_ef';re.exec(str);// → matches 'a0b*c'
...
![](/_img/regexp-tier-up/results-memory.svg)
...
![](/_img/regexp-tier-up/results-speed.svg)

Figure 2: RegExp performance comparison

Figure 2 shows the impact on the performance of the RegExp interpreter for all improvements described in this blog post[\[1\]](#fn1) on the RexBench benchmark suite. For reference, the performance of JIT compiled RegExp is also shown (Native).
...
1.  The results shown here also include an improvement to regular expressions already described in the [V8 v7.8 release notes](/blog/v8-release-78#faster-regexp-match-failures). [↩︎](#fnref1)