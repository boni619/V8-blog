...
Published 15 February 2021 · Tagged with [internals](/blog/tags/internals)
...
TL;DR: Reverse the arguments [#](#tl%3Bdr%3A-reverse-the-arguments)
-------------------------------------------------------------------
...
A typical JavaScript stack frame before and after removing the arguments adaptor frame.

Making JavaScript calls faster [#](#making-javascript-calls-faster)
-------------------------------------------------------------------
...
State of the frame after the execution of `InterpreterPushArgsThenCall` built-in.
...
The `InterpreterFrame` for the call `add42(3)`.
...
State of the frames after the execution of `InterpreterPushArgsThenCall` built-in.
...
Stack frames with arguments adaptation.
...
State of the frames after the execution of the callee `add42`.
...
Removing the arguments adaptor frame [#](#removing-the-arguments-adaptor-frame)
-------------------------------------------------------------------------------
...
### Arguments in the callee frame [#](#arguments-in-the-callee-frame)
...
Stack frames without arguments adaptors.
...
### TurboFan with the arguments adaptor frame [#](#turbofan-with-the-arguments-adaptor-frame)
...
### TurboFan without the arguments adaptor frame [#](#turbofan-without-the-arguments-adaptor-frame)
